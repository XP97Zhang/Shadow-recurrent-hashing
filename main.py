...
def compute_u(b):
    one = torch.ones_like(b.data)
    return torch.where(b.data > 0, one, -one)

def hashing_loss(b, cls, m, alpha, beta):
    """
    ...
    loss = loss.mean() + alpha / 2 * ((b - u) ** 2).sum(dim=1).mean() \
    + beta / 2 * (((b ** 2).sum(dim=1) - m / 2) ** 2).mean()

    return loss

@timing
def train(epoch, dataloader, net, optimizer, m, alpha, beta):
    accum_loss = 0
    net.train()
    for i, (img, cls) in enumerate(dataloader):
        ...
        net.zero_grad()
        b = net(img)
        loss = hashing_loss(b, cls, m, alpha, beta)

        loss.backward()
        optimizer.step()
        accum_loss += loss.data.item()

        not i%20 and print(f'[{epoch}][{i}/{len(dataloader)}] loss: {loss.data.item():.4f}')
    return accum_loss / len(dataloader)

@timing
def test(epoch, dataloader, net, m, alpha, beta):
    accum_loss = 0
    net.eval()
...
def main():
    ...
    parser.add_argument('--beta', type=float, default=0.01, help='weighting of regularizer')
    ...
    for epoch in range(resume_epoch, opt.niter):
        train_loss = train(epoch, train_loader, net, optimizer, 2 * opt.binary_bits, opt.alpha, opt.beta)
        logger.add_scalar('train_loss', train_loss, epoch)

        test_loss = test(epoch, test_loader, net, 2 * opt.binary_bits, opt.alpha, opt.beta)
        logger.add_scalar('test_loss', test_loss, epoch)

        if epoch % opt.checkpoint == 0:
            # compute mAP by searching testset images from trainset
            print(f'[{epoch}]computing result...')
            trn_binary, trn_label = compute_result(train_loader, net)
            tst_binary, tst_label = compute_result(test_loader, net)
            print(f'[{epoch}]computing mAP...')
